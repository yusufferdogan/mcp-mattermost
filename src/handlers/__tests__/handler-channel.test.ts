...
describe('HandlerChannel', () => {
  ...
  it('should search channels', async () => {
    const tools = handler.getMcpTools();
    const searchChannelsTool = tools.find(tool => tool.name === 'search_channels');
    if (searchChannelsTool) {
      ...
      fail('search_channels tool not found');
    }
  });

  it('should get channels by channel ID', async () => {
    const tools = handler.getMcpTools();
    const getChannelsTool = tools.find(tool => tool.name === 'get_channels');
    if (getChannelsTool) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      fail('get_channels tool not found');
    }
  });
